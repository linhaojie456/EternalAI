...
class TokenizerHelper(private val modelDir: File) {
    ...
    init {
        try {
                ...
                if (vocabStart >= 0) {
                    ...
                    if (openingBrace >= 0) {
                        ...
                        for (i in openingBrace until vocabSection.length) {
                            val c = vocabSection[i]; sb.append(c)
                            if (c == '{') braceCount++ else if (c == '}') { braceCount--; if (braceCount == 0) break }
                        }
                        ...
                        for (entry in entries) {
                            ...
                            if (colon >= 0) {
                                val token = entry.substring(0, colon).trim().removeSurrounding("\"")
                                val idStr = entry.substring(colon + 1).trim()
                                try { val id = idStr.toLong(); vocab[token] = id; idToToken[id] = token } catch (_: Exception) {}
                            }
                        }
                    }
                }
            ...
            if (configFile.exists()) {
                ...
                if (matchResult != null) { val id = matchResult.groupValues[1].toLongOrNull(); if (id != null) eosTokenId = id }
            }
    ...
    fun encode(text: String): LongArray {
        ...
        while (remaining.isNotEmpty()) {
            ...
            for ((token, id) in vocab) {
                if (remaining.startsWith(token)) { ids.add(id); remaining = remaining.substring(token.length); found = true; break }
            }
            if (!found) { ids.add(remaining[0].code.toLong()); remaining = remaining.substring(1) }
        }
    ...
    fun decode(ids: LongArray): String {
        val sb = StringBuilder()
        for (id in ids) { val token = idToToken[id]; if (token != null) sb.append(token) else sb.append(id.toChar()) }
        return sb.toString()
    }
}
